...
int ll2string(char *s, size_t len, long long value) {
	...
	//从右向左将每一个value数字转换成char
    do {
        *p-- = '0'+(v%10);
        v /= 10;
    } while(v);
    if (value < 0) *p-- = '-';	//<0 最前面添加'-'
    p++;	//p指向字符串开始位置
    l = 32-(p-buf);	//字符串长度
    if (l+1 > len) l = len-1; /* Make sure it fits, including the nul term */ //超过给定长度，截断
    memcpy(s,p,l);
    s[l] = '\0';	//字符串结尾
    return l;		//返回字符串长度
}
